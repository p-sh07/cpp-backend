...
int main(int argc, const char* argv[]) {
    ...
    try {
        ...
        // 2. Загружаем игру из файла конфига
        auto game_app = std::make_shared<app::GameApp>(
            std::move(json_loader::LoadGame(args->config_path)), ioc
        );
        game_app->SetRandomDogSpawnPoint(args->randomize_spawn_points);
