...
/*
TEST_CASE("Four gatherers, collect radius test", "[CollisionDetect]") {
    //TODO: Calculate:
    //Calculated manually:
    EventVector CorrectResult = {
        {0, 0, 0, 0.0},
    ...
    //To use with function:
    const std::vector<Item> items {
        {{4,1}, 0}, //loot item from game - max dist 0.3
        {{4,0.55}, 0.3}, //office from game - max dist 0.55
    };

    const std::vector<Gatherer> gatherers {
        {{0, 1.00}, {4, 1.00}, 0.6}, // dist: office = 0.45 [ok], item = 0 [ok], time = 1.0
        {{1, 0.70}, {5, 0.70}, 0.6}, // dist: office = 0.15 [ok], item = 0.3 [edge case], time 0.75
        {{3, 0.00}, {7, 0.00}, 0.6}, // dist: office = 0.55 [edge case], item = 0. [not ok], time 0.25
        {{4,-0.01}, {8,-0.01}, 0.6}, // dist: to item & office = 0.56 [not ok], time 0
    };
